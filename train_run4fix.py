#!/usr/bin/env python3
"""
Oncoformer DNA-only pretraining  —  MLM-only  —  BUGFIX of run4

Identical to run4 (train.py) EXCEPT the single-task uncertainty-weighting bug is
fixed: log_var_recon is frozen at 0 so the total loss == recon (MLM) loss. Fresh
OUT_DIR (run4_fixed_out) => from scratch; run4's original results are preserved.
Heads: gene + aa_vaf_bin only.  DINO: disabled (w_dino = 0.0).
"""
...
# ---------------------------------------------------------------------------
# BUGFIX — single-task heteroscedastic uncertainty weighting.
# With DINO disabled (w_dino=0) the loss reduces to  exp(-s)*L_recon + s , where
# s = model.log_var_recon is a free nn.Parameter (init 0, clamp [-2,3]). With only
# one task this is degenerate: s drifts toward its -2 floor, so the logged loss
# slides negative independently of task quality AND the effective MLM LR grows
# (prec = exp(-s): 1 -> 1.28 (run5) / 1.44 (run4), heading to 7.4x at the floor).
# Fix: freeze s = 0  =>  prec = 1  =>  total loss == recon_meta_w * L_recon == L_recon.
# OncoformerOmics.configure_optimizers() skips params with requires_grad=False
# (models.py:2008), so s is also excluded from the optimizer and stays 0 for the
# entire run (including preemption resumes, since this block re-runs each launch).
# ---------------------------------------------------------------------------
