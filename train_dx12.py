#!/usr/bin/env python3
"""
Oncoformer DNA-only pretraining  —  MLM-only  —  DX1/DX2 baitset ONLY, FROM SCRATCH

Same architecture / hyperparameters as run4 (train.py); differs only in:
  - trains on DX1/DX2 baitset records only (via dx12_config baitset_filter)
  - fresh OUT_DIR (run5_dx12_out) => no last.ckpt => starts from scratch
