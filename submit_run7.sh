#!/bin/bash
#SBATCH -J onco_mlm_run7
#SBATCH -o /cv/home/wangs278/scratch/fmi/run7_moco_out/logs/train_%J.stdout
#SBATCH -e /cv/home/wangs278/scratch/fmi/run7_moco_out/logs/train_%J.stderr
...
# NCCL_DEBUG=INFO: job 17848085 hung 2h+ in ncclCommInitRank (never printed the
# "NCCL version" line) on the busy shared node -110. INFO traces the init stages so a
# recurrence is diagnosable; it is verbose at init only and quiet during steady training.
...
SCRIPT=/cv/home/wangs278/scratch/fmi/train_run7.py
...
# Stage to /dev/shm for fast multi-GPU reads. run7-specific shm basename avoids collision
# with a co-scheduled run6 job. Abort if the cache is missing (else 4 ranks race to tokenize).
RUN_CACHE=${DATA_DIR}/tokenized_dna_dx12_moco.pt
SHM_COPY=/dev/shm/onco_tokenized_dna_dx12_moco_run7_${SLURM_JOB_ID}.pt
if [ -f "${RUN_CACHE}" ]; then
...
# job (--requeue) so it resumes from run7_moco_out/checkpoints/last.ckpt. A genuine
...
set +e
# -u: unbuffered stdout. Without it, block-buffering hid all of train_run7.py's startup
# prints during job 17848085, making the NCCL-init hang impossible to localise from logs.
python -u ${SCRIPT}
TRAIN_EXIT=$?
set -e
