#!/bin/bash
#SBATCH -J onco_mlm_run6
#SBATCH -o /cv/home/wangs278/scratch/fmi/run6_moco_out/logs/train_%J.stdout
#SBATCH -e /cv/home/wangs278/scratch/fmi/run6_moco_out/logs/train_%J.stderr
...
SCRIPT=/cv/home/wangs278/scratch/fmi/train_run6.py
...
# Cache DX1/DX2 MoCo tokenized data to /dev/shm for fast multi-GPU reads.
# Built once by pretokenize_run6.py — abort if missing (else 4 DDP ranks race to tokenize).
RUN6_CACHE=${DATA_DIR}/tokenized_dna_dx12_moco.pt
SHM_COPY=/dev/shm/onco_tokenized_dna_dx12_moco_${SLURM_JOB_ID}.pt
if [ -f "${RUN6_CACHE}" ]; then
    echo "=== Caching ${RUN6_CACHE} to /dev/shm ($(du -sh ${RUN6_CACHE} | cut -f1)) ==="
    cp "${RUN6_CACHE}" "${SHM_COPY}"
    trap "rm -f ${SHM_COPY}" EXIT
    echo "=== Cache ready ==="
else
    echo "=== ERROR: ${RUN6_CACHE} missing. Run submit_pretok_run6.sh first. ==="
    exit 1
fi

# Preemption/timeout handling: SLURM sends SIGTERM before preempting. Requeue the
# SAME job (--requeue in the header) so it resumes from run6_moco_out/checkpoints/last.ckpt.
