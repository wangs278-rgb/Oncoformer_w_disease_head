#!/usr/bin/env python3
"""Violin plot (Arial) of in-silico gene-knockout ΔP for the top-influence genes.

x-axis : top genes (by mean TV influence)
y-axis : ΔP = P_knockout - P_baseline  (one value per disease lineage, mean over carriers)
violin : distribution of ΔP across all 426 supervised disease classes for that gene
dots   : one per lineage, jittered; colored by lineage for the set of "top lineages"
         (each selected gene's single most-lowered cancer), light-grey otherwise.
...
# ---- Arial (match the other KO figures) ----
...
genes = d['genes']
meandP = d['meandP']           # [G, S] mean ΔP per gene x lineage
tv = d['influence_tv']
dnames = np.array([str(x) for x in d['sup_names']])
G, S = meandP.shape

TOPN = 25
o = np.argsort(-tv)[:TOPN]                      # gene indices, most influential first
gsel = genes[o]
M = meandP[o]                                   # [TOPN, S]
...
# ---- "top lineages" = union of each selected gene's single most-lowered cancer ----
top_line_idx = []
for r in range(TOPN):
    j = int(np.argmin(M[r]))                    # most-lowered lineage for this gene
    if j not in top_line_idx:
        top_line_idx.append(j)
# stable, high-contrast palette (grey-free so dots never blend into the background swarm)
...
# violins (distribution across all 426 lineages)
...
# jittered dots, one per lineage
rng = np.random.default_rng(0)
for r in range(TOPN):
    vals = M[r]
    jit = rng.uniform(-0.30, 0.30, size=S)
    is_top = np.array([j in top_set for j in range(S)])
    # background (non-top lineages)
    ax.scatter(x[r] + jit[~is_top], vals[~is_top], s=5, c='#c9c9c9',
               alpha=0.30, linewidths=0, zorder=2)
    # highlighted top lineages
    tj = np.where(is_top)[0]
    ax.scatter(x[r] + jit[tj], vals[tj], s=22,
               c=[color_of[j] for j in tj], alpha=0.95,
               edgecolors='white', linewidths=0.4, zorder=4)

ax.axhline(0, color='k', lw=0.7, zorder=1)
ax.set_xticks(x)
ax.set_xticklabels(gsel, rotation=90, fontsize=8)
ax.set_ylabel('ΔP  =  P$_{knockout}$ − P$_{baseline}$   (per disease lineage)', fontsize=9)
ax.set_title('In-silico gene knockout: per-lineage ΔP distribution across the top-influence genes\n'
             'each dot = one of 426 disease classes; colored dots = the top-driven cancers '
             '(negative = knockout lowers that cancer)', fontsize=10)
ax.tick_params(axis='y', labelsize=8)
for s in ('top', 'right'):
    ax.spines[s].set_visible(False)
ax.set_xlim(-0.7, TOPN - 0.3)

# legend of top lineages
...
ax.legend(handles=handles, title='Top-driven cancer lineages',
          fontsize=6.6, title_fontsize=7.5, loc='center left',
          bbox_to_anchor=(1.005, 0.5), frameon=False, labelspacing=0.35)

fig.savefig(f'{FIG}/fig5_gene_violin.pdf')
fig.savefig(f'{FIG}/fig5_gene_violin.png')
plt.close(fig)
print(f'wrote fig5_gene_violin (PDF+PNG) — {TOPN} genes, {S} lineages, '
      f'{len(top_line_idx)} highlighted lineages')
print('highlighted lineages:')
for j in top_line_idx:
    print('  ', dnames[j])
